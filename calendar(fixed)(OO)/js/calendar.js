...
var calendar = {
	...
	showMons: function(e){
		if (!document.getElementById("cal-mons")) {
			...
			calMons.onclick = calendar.chooseMon;
	...
	//选择月份
	chooseMon: function(e){
		e = e ? e : window.e;
		var target = e.target || e.srcElement;
		now.setMonth(target.firstChild.nodeValue - 1);  //设置月份，从0开始索引值对应月份
		calendar.updateCal(now);		//更新显示
		calendar.showMons(e);			//隐藏选项
	},
	//显示隐藏年份
	showYears: function(e){
		if (!document.getElementById("cal-years")) {	
			...
			calYears.onclick = calendar.chooseYear;
	...
	chooseYear: function(e){
		...
		calendar.showYears(e);	//当点击左右箭头时，为再次调用函数添加选项；当点击选项时，为调用函数移除选项
		calendar.updateCal(now);									//更新年份
	}
};